...
ORIGINAL_DIR = Path("original_image")
PROCESSED_DIR = Path("processed_image")


# remove Session -> we don't know if they are thread safe or not
def download_single_image(url: str, img_num: int) -> Path:
    print(f"Downloading {url}...")
    ts = int(time.time())
    url = f"{url}?ts={ts}"
    # instead create new session for each url
    ...
    print(f"Donwloaded and save to: {download_path}")
...
async def download_images(urls: list) -> list[Path]:
    ...
    # Task Gather
    # tasks = [
    #     asyncio.create_task(asyncio.to_thread(download_single_image, url, img_num))
    #     for img_num, url in enumerate(urls, start=1)
    # ]
    # img_paths = await asyncio.gather(*tasks,return_exceptions=False)
...
def process_single_image(orig_path: Path) -> Path:
    ...
    with Image.open(orig_path) as img:
        data = list(img.getdata())  # pyright: ignore
...
# This is CPUbound so using thread will not change the final result
async def process_images(orig_paths: list[Path]) -> list[Path]:
    # Task Group + Convert to Thread (to have impact you need to convert it to process cause this is cpubound not iobound)
    async with asyncio.TaskGroup() as tg:
        tasks = [
    ...
    # Task Gather
    # tasks = [
    #     asyncio.create_task(asyncio.to_thread(process_single_image, orig_path))
    #     for orig_path in orig_paths
    # ]
    # img_paths = asyncio.gather(*tasks, return_exceptions=False)
...
async def process_images_executer(orig_paths: list[Path]) -> list[Path]:
    loop = asyncio.get_running_loop()
    with ProcessPoolExecutor() as executer:
        tasks = [
            loop.run_in_executor(executer, process_single_image, orig_path)
            for orig_path in orig_paths
        ]
    img_paths = [await task for task in tasks]
    return img_paths
...
async def main():
    ...
    # With Process
    processed_paths = await process_images_executer(img_paths)
