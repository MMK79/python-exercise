...
async def download_single_image(
    ...
    async with semaphore:
        ...
        async with aiofiles.open(download_path, "wb") as f:
            # Asynchronous Iterator -> it now wait for each chunk
...
async def download_images(urls: list) -> list[Path]:
    # Same client for every download
...
async def process_images(orig_paths: list[Path]) -> list[Path]:
    ...
    processed_paths = await asyncio.gather(*tasks, return_exceptions=False)
...
async def process_images_executer(orig_paths: list[Path]) -> list[Path]:
    loop = asyncio.get_running_loop()
    with ProcessPoolExecutor(max_workers=CPU_WORKERS) as executer:
        tasks = [
            loop.run_in_executor(executer, process_single_image, orig_path)
            for orig_path in orig_paths
        ]
